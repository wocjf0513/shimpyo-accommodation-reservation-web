    ...
    void shouldSuccessToGetAllProducts() throws Exception {
        ...
        getProductAction
            ...
            .andDo(restDoc.document(
                queryParameters(
                    parameterWithName("page").optional().description("페이지 인덱스"),
                ...
                responseFields(
                    ...
                    fieldWithPath("data[].category").type(JsonFieldType.STRING).description("상품 카테고리(호텔, 모텔, 풀빌라, 펜션)"),
                    ...
                    fieldWithPath("data[].image").type(JsonFieldType.STRING).description("상품 썸네일 이미지"),
                    fieldWithPath("data[].price").type(JsonFieldType.NUMBER).description("상품 내 방 최저 가격")
                )
            ));
...
//            reservationProductRepository.save(ProductFactory.createTestReservationProduct(room, reservation));
        ...
        // when
        ResultActions getProductAction = mockMvc.perform(
            get("/api/products/{productId}?startDate=2023-11-22&endDate=2023-11-23",product.getId()));
        ...
        getProductAction
            .andDo(restDoc.document(
                ...
                responseFields(
                    ...
                    fieldWithPath("data.rooms").type(JsonFieldType.ARRAY).description("상품 하위 방 데이터"),
                    fieldWithPath("data.rooms[].roomId").type(JsonFieldType.NUMBER).description("방 아이디"),
                    fieldWithPath("data.rooms[].roomName").type(JsonFieldType.STRING).description("방 이름"),
                    fieldWithPath("data.rooms[].price").type(JsonFieldType.NUMBER).description("방 가격"),
                    ...
                    fieldWithPath("data.rooms[].checkOut").type(JsonFieldType.STRING)
                        .description("체크아웃 시간"),
                    fieldWithPath("data.rooms[].reserved").type(JsonFieldType.BOOLEAN).description("예약 여부")
                )
            ));
